...
void *worker1_func(void *arg) {
    for (int i = 0; i < WORKER_LOOPS; ++i) {
        ...
        sem_wait(&sem_lista); // reduz o semáforo, esperando que o worker 2 libere
        printf("Worker 1 obteve sem_lista\n");

        int operacao = operacao_worker1();

        pthread_mutex_lock(&mutex_total); // trava o mutex para acessar a variável global
        printf("Worker 1 obteve mutex_total\n");

        total_computado += operacao;
        lista_de_operacoes[proximo_indice++] = operacao;

        pthread_mutex_unlock(&mutex_total); // libera o mutex
        printf("Worker 1 liberou mutex_total\n");

        sem_post(&sem_lista); // libera o semáforo para o worker 2
...
void *worker2_func(void *arg) {
    for (int i = 0; i < WORKER_LOOPS; ++i) {
        ...
        sem_wait(&sem_lista); // reduz o semáforo, esperando que o worker 1 libere
        ...
        pthread_mutex_unlock(&mutex_total); // libera o mutex para o worker 1
        printf("Worker 2 liberou mutex_total\n");

        sem_post(&sem_lista); // libera o semáforo para o worker 1
