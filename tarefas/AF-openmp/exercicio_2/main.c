...
void init_matrix(double* m, int rows, int columns) {
    #pragma omp parallel for schedule(static) // essa parte estava certa
...
void mult_matrix(double* out, double* left, double *right, 
    ...
    // indicamos que o (i, j, k) são privados para cada thread
    // e que a divisão do trabalho será feita de forma estática
    for (i = 0; i < rows_left; ++i) {
        for (j = 0; j < cols_right; ++j) {
            out[i*cols_right+j] = 0;
            // removemos o pragma aqui
